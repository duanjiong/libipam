...
func (rw blockReaderWriter) getAffineBlocks(ctx context.Context, host string, ver int, id uint32, pools []v1alpha1.IPPool) (blocksInPool, blocksNotInPool []cnet.IPNet, err error) {
...
func (rw blockReaderWriter) findUnclaimedBlock(ctx context.Context, host string, version int, pools []v1alpha1.IPPool, config IPAMConfig) (*cnet.IPNet, error) {
...
func (rw blockReaderWriter) getPendingAffinity(ctx context.Context, host string, id uint32, subnet cnet.IPNet) (*model.KVPair, error) {
	logCtx := log.WithFields(log.Fields{"host": host, "subnet": subnet})
	logCtx.Info("Trying to create affinity in pending state")
	obj := model.KVPair{
		Key:   model.BlockAffinityKey{Host: host, CIDR: subnet, ID: id},
	...
	if err != nil {
		...
		aff, err = rw.queryAffinity(ctx, host, id, subnet, "")
		if err != nil {
...
func (rw blockReaderWriter) claimAffineBlock(ctx context.Context, aff *model.KVPair, config IPAMConfig, rsvdAttr *HostReservedAttr) (*model.KVPair, error) {
	...
	block := newBlock(id, subnet, rsvdAttr)
	block.Affinity = &affinityKeyStr

	// Create the new block in the datastore.
	o := model.KVPair{
		Key:   model.BlockKey{CIDR: block.CIDR, ID: id},
	...
	if err != nil {
		if _, ok := err.(cerrors.ErrorResourceAlreadyExists); ok {
			...
			obj, err := rw.queryBlock(ctx, id, subnet, "")
			if err != nil {
...
func (rw blockReaderWriter) confirmAffinity(ctx context.Context, aff *model.KVPair) (*model.KVPair, error) {
	...
	if err != nil {
		...
		// another process.
		kvp, err2 := rw.queryAffinity(ctx, host, id, cidr, "")
...
func (rw blockReaderWriter) releaseBlockAffinity(ctx context.Context, host string, id uint32, blockCIDR cnet.IPNet, requireEmpty bool) error {
	...
	aff, err := rw.queryAffinity(ctx, host, id, blockCIDR, "")
	if err != nil {
	...
	obj, err := rw.queryBlock(ctx, id, blockCIDR, "")
	if err != nil {
...
func (rw blockReaderWriter) queryAffinity(ctx context.Context, host string, id uint32, cidr cnet.IPNet, revision string) (*model.KVPair, error) {
	return rw.client.Get(ctx, model.BlockAffinityKey{Host: host, ID: id, CIDR: cidr}, revision)
}
...
func (rw blockReaderWriter) queryBlock(ctx context.Context, id uint32, blockCIDR cnet.IPNet, revision string) (*model.KVPair, error) {
	return rw.client.Get(ctx, model.BlockKey{ID: id, CIDR: blockCIDR}, revision)
}
...
func (rw blockReaderWriter) getPoolForIP(id uint32, ip cnet.IP, enabledPools []v1alpha1.IPPool) (*v1alpha1.IPPool, error) {
	if enabledPools == nil {
		var err error
...
func blockGenerator(pool *v1alpha1.IPPool, cidr cnet.IPNet) func() *cnet.IPNet {
...
// the it returns nil.
func randomBlockGenerator(ipPool v1alpha1.IPPool, hostName string) func() *cnet.IPNet {
...
func (rw blockReaderWriter) getBlockForIP(ctx context.Context, id uint32, ip cnet.IP) (*cnet.IPNet, error) {
	...
	for _, o := range datastoreObjs.KVPairs {
		...
		if k.CIDR.IPNet.Contains(ip.IP) && k.ID == id {
