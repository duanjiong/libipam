...
func (h allocationHandle) incrementBlock(id uint32, blockCidr cnet.IPNet, num int) int {
	blockId := fmt.Sprintf("%d-%s", id, blockCidr.String())
	newNum := num
...
func (h allocationHandle) decrementBlock(id uint32, blockCidr cnet.IPNet, num int) (*int, error) {
	blockId := fmt.Sprintf("%d-%s", id, blockCidr.String())
