...
const (
	...
	BlockAffinityCRDName      = "blockaffinities.network.kubesphere.io"
)
...
func NewBlockAffinityClient(c *kubernetes.Clientset, r *rest.RESTClient) K8sResourceClient {
	...
	rc := customK8sResourceClient{
		...
		description:     "Kubesphere IPAM block affinities",
		k8sResourceType: reflect.TypeOf(v1alpha1.BlockAffinity{}),
		k8sResourceTypeMeta: metav1.TypeMeta{
			Kind:       v1alpha1.KindBlockAffinity,
			APIVersion: v1alpha1.GroupVersionCurrent,
		},
		k8sListType:  reflect.TypeOf(v1alpha1.BlockAffinityList{}),
		resourceKind: v1alpha1.KindBlockAffinity,
	}
...
func (c blockAffinityClient) toV1(kvpv3 *model.KVPair) (*model.KVPair, error) {
	...
	_, cidr, err := net.ParseCIDR(kvpv3.Value.(*v1alpha1.BlockAffinity).Spec.CIDR)
	if err != nil {
		log.WithField("cidr", cidr).WithError(err).Error("failed to parse cidr")
		return nil, err
	}
	id := kvpv3.Value.(*v1alpha1.BlockAffinity).Spec.ID
	state := model.BlockAffinityState(kvpv3.Value.(*v1alpha1.BlockAffinity).Spec.State)

	// Determine deleted status.
	deletedString := kvpv3.Value.(*v1alpha1.BlockAffinity).Spec.Deleted
	del := false
	if deletedString != "" {
	...
	return &model.KVPair{
		Key: model.BlockAffinityKey{
			CIDR: *cidr,
			ID:   id,
			Host: kvpv3.Value.(*v1alpha1.BlockAffinity).Spec.Node,
		},
...
func (c blockAffinityClient) toV3(kvpv1 *model.KVPair) *model.KVPair {
	name, cidr, host, id := c.parseKey(kvpv1.Key)
	...
	return &model.KVPair{
		...
		Value: &v1alpha1.BlockAffinity{
			...
			Spec: v1alpha1.BlockAffinitySpec{
				State:   string(state),
				Node:    host,
				ID:      id,
				CIDR:    cidr,
...
func (c *blockAffinityClient) DeleteKVP(ctx context.Context, kvp *model.KVPair) (*model.KVPair, error) {
	...
	k := model.ResourceKey{Name: name, Kind: v1alpha1.KindBlockAffinity}
...
func (c *blockAffinityClient) List(ctx context.Context, list model.ListInterface, revision string) (*model.KVPairList, error) {
	l := model.ResourceListOptions{Kind: v1alpha1.KindBlockAffinity}
...
func (c *blockAffinityClient) Watch(ctx context.Context, list model.ListInterface, revision string) (api.WatchInterface, error) {
	resl := model.ResourceListOptions{Kind: v1alpha1.KindBlockAffinity}
